...
1. **Step 1**: The AI client describes the refactoring plan and begins analyzing code structure
...
After the AI client completes the investigation (unless confidence is **complete**):
...
💡**Example of a powerful prompt** to get the best out of a deep + fast model pairing (e.g., Kimi thinking + GLM flash):
```
...
This results in the primary client model first performing its own expert analysis, encouraging critical thinking and identifying links within the project code. It then prompts the fast model to review the same code with a hint—preventing duplication and encouraging it to explore other areas not already discovered.
...
- **Precise line-number references** - Provides exact line numbers for the client to implement changes
