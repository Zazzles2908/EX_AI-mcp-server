...
The `debug` workflow guides the AI client through a systematic investigation process where the client performs methodical code
...
The debug tool implements a **systematic investigation methodology** where the AI client is guided through structured debugging steps:

**Investigation Phase:**
1. **Step 1**: The AI client describes the issue and begins thinking deeply about possible underlying causes, side-effects, and contributing factors
...
This structured approach ensures the AI client performs methodical groundwork before expert analysis, resulting in significantly better debugging outcomes and more efficient token usage.
