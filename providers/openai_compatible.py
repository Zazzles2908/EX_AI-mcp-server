...
class OpenAICompatibleProvider(ModelProvider):
    ...
    def generate_content(
        ...
        **kwargs,
        ...
        # Optional Moonshot/Kimi file attachment support via 'input_file' content parts
        file_ids = kwargs.get("file_ids") or []
        use_input_file_parts = bool(kwargs.get("use_input_file_parts", False))
        if file_ids and use_input_file_parts:
            for fid in file_ids:
                try:
        ...
        # Log sanitized payload for observability (no user secrets)
        try:
            import json as _json
            def _san(v):
                if isinstance(v, dict):
                    return {k: _san(v) for k, v in v.items() if k not in {"api_key", "Authorization"}}
                if isinstance(v, list):
                    return [_san(x) for x in v]
                return v
            logging.info("chat.completions.create payload (sanitized): %s", _json.dumps(_san(completion_params), ensure_ascii=False))
        except Exception:
            pass
        ...
        # Add tools/tool_choice and streaming if present; plus other OpenAI params
        # Use capabilities to filter parameters for reasoning models
        # tools: list of OpenAI tool specs; tool_choice: 'auto'|'none'|{'type':'function','function':{'name':...}}
        ...
        # Stream pass-through (True/False)
        if "stream" in kwargs:
            completion_params["stream"] = kwargs["stream"]
        # Extra headers/body passthrough (for caching or vendor-specific controls)
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # Streaming support: accumulate chunks into final content when stream=True
                if completion_params.get("stream") is True:
                    ...
                    try:
                        for event in response:
                            try:
                                ...
                                # Some providers emit 'message' in chunks
                                msg = getattr(choice, "message", None)
                                if msg and getattr(msg, "content", None):
                                    content_parts.append(msg.content)
                                # Capture model/id/created from first chunk if present
                    ...
                    usage = None  # Streaming chunks may not include usage
                    return ModelResponse(
                        content=content,
                        usage=usage,
                        model_name=model_name,
                ...
                # Non-streaming path (robust across OpenAI-compatible providers)
                # Some providers (e.g., Moonshot Kimi) may omit `message` or nest content differently.
                choice0 = None
                try:
                ...
                if not content and choice0 is not None:
                    try:
                        ...
                        pass
                if not content:
                    # Last resort: top-level content (rare)
                ...
                return ModelResponse(
                    ...
                    provider=self.get_provider_type(),
                    metadata={
                        "finish_reason": getattr(choice0, "finish_reason", None) or getattr(getattr(response, "choices", [{}])[0], "finish_reason", None) or "Unknown",
