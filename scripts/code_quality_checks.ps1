...
Write-Emoji "🔍" "Running Code Quality Checks for EX MCP Server" -Color Cyan
...
foreach ($tool in $devTools) {
    ...
    # Check in venv
    if ($IsWindows -or $env:OS -eq "Windows_NT") {
        if (Test-Path ".venv\Scripts\$tool.exe") {
            $toolFound = $true
        }
    } else {
        if (Test-Path ".venv/bin/$tool") {
            $toolFound = $true
        }
    }
...
# Set tool paths
if ($IsWindows -or $env:OS -eq "Windows_NT") {
    $ruffCmd = if (Test-Path ".venv\Scripts\ruff.exe") { ".venv\Scripts\ruff.exe" } else { "ruff" }
    $blackCmd = if (Test-Path ".venv\Scripts\black.exe") { ".venv\Scripts\black.exe" } else { "black" }
    $isortCmd = if (Test-Path ".venv\Scripts\isort.exe") { ".venv\Scripts\isort.exe" } else { "isort" }
    $pytestCmd = if (Test-Path ".venv\Scripts\pytest.exe") { ".venv\Scripts\pytest.exe" } else { "pytest" }
} else {
    $ruffCmd = if (Test-Path ".venv/bin/ruff") { ".venv/bin/ruff" } else { "ruff" }
    $blackCmd = if (Test-Path ".venv/bin/black") { ".venv/bin/black" } else { "black" }
    $isortCmd = if (Test-Path ".venv/bin/isort") { ".venv/bin/isort" } else { "isort" }
    $pytestCmd = if (Test-Path ".venv/bin/pytest") { ".venv/bin/pytest" } else { "pytest" }
}
