...
if __name__ == "__main__":
    ...
    target = pathlib.Path(__file__).parent / "ws" / "ws_status.py"
    try:
...
#!/usr/bin/env python
"""
WS Daemon Status Helper

Reads logs/ws_daemon.health.json and prints a concise status line:
- running/stale (based on timestamp freshness)
- host:port
- pid (if present)
- sessions, inflight/capacity

Exit codes:
- 0: health file present and recent (<= 20s)
- 1: health file missing or stale
"""
import json
import os
import sys
...
def main() -> int:
    if not HEALTH.exists():
        print("ws_status: health file not found:", HEALTH)
        return 1
    try:
        data = json.loads(HEALTH.read_text(encoding="utf-8"))
    except Exception as e:
        print("ws_status: failed to read health file:", e)
        return 1
    ...
    inflight = data.get("global_inflight")
    cap = data.get("global_capacity")

    state = "running" if age <= FRESH_SECS else f"stale ({int(age)}s old)"
