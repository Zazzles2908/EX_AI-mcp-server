#!/usr/bin/env python
"""
Copy of scripts/ws_chat_once.py under scripts/ws.
"""
...
def _extract_text_and_content(outputs: Any) -> Tuple[str, str]:
    texts: List[str] = []; content_field: str = ""
    ...
    def walk(x: Any) -> None:
        if isinstance(x, dict):
            if x.get("type") == "text" and isinstance(x.get("text"), str):
                s = x["text"]; texts.append(s); maybe_parse_tooloutput_text(s)
...
async def call_chat(ws, model: str, prompt: str, timeout: float = 180.0):
    ...
    while True:
        ...
        if msg.get("op") == "call_tool_res" and msg.get("request_id") == req_id:
            if msg.get("error"): return False, str(msg.get("error")), None, None
...
async def main() -> int:
    if len(sys.argv) < 3:
        print("Usage: python scripts/ws/ws_chat_once.py <model> <prompt>"); return 64
    model = sys.argv[1]; prompt = " ".join(sys.argv[2:])
    uri = f"ws://{HOST}:{PORT}"
    try:
        async with websockets.connect(uri, max_size=20 * 1024 * 1024) as ws:
            ...
            if not ack.get("ok"):
                print("[WS] Auth failed:", ack); return 2
